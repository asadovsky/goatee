...
func wsHandler(h *hub, ws *websocket.Conn) {
	...
	PanicOnError(websocket.Message.Send(ws, marshalOrPanic(nc)))
	...
	go func() {
		for {
			...
			var u Update
			PanicOnError(json.Unmarshal(b, &u))
			...
			for i := u.BaseTxnId + 1; i < len(h.txns); i++ {
				...
				Assert(txn.clientId != u.ClientId)
				ops, _ = TransformCompound(ops, txn.ops)
			}
...
func rootHandler(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasSuffix(name, "html") {
		...
		PanicOnError(tmpl.ExecuteTemplate(w, filepath.Base(name), data))
	} else {
