...
module.exports = function(docId, onDocLoaded) {
  /* jshint nonew: false */
...
function Document(onDocLoaded) {
  ...
  this.socket_.onmessage = (function(event) {
    ...
    var ops = otText.opsFromStrings(msg['OpStrs']);
    var tup = otText.transformCompound(
...
Model.prototype.insertText = function(pos, value) {
  ...
  this.doc_.pushOp_(new otText.Insert(pos, value));
};

Model.prototype.deleteText = function(pos, len) {
  if (len === 0) return;
  this.doc_.pushOp_(new otText.Delete(pos, len));
};
...
Model.prototype.apply_ = function(op, isLocal) {
  ...
  case 'Insert':
    ...
    this.dispatchEvent_(new events.InsertTextEvent(isLocal, op.pos, op.value));
    break;
  case 'Delete':
    ...
    this.dispatchEvent_(new events.DeleteTextEvent(isLocal, op.pos, op.len));
    break;
  default:
